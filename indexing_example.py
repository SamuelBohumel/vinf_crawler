...
from lucene import NIOFSDirectory, Document, Field,\
...
# TODO: currently not part of PyLucene:
...
class SimpleIndexer(object):
    ...
    def index (cls, indexDir, taxoDir):
        ...
        """
        # print("SimpleIndexer: indexDir=%s, taxoDir=%s" % (indexDir, taxoDir)
        ...
        for docNum in range(len(docTexts)):
            ...
            print("facets for doc#%d : %s" %(docNum, facets))
            facetList = [ createCategoryPath(f) for f in facets]
            print("facetList of size %d : %s" % (len(facetList),facetList))
            ...
            ## print("facetList#2:",facetList, type(facetList) ,type(facetList.get(0))

            # TODO: JavaList is currently not part of PyLucene:
            # Alternatively we could convert our Python list to a Java based list
            # using the JavaList class defined by lucene.collections. (JavaList implements
            # java.util.List around a Python list instance it wraps.)
            # facetList = JavaList(facetList)
            # print("facetList",facetList, type(facetList) ,type(facetList.get(0))
...
from lucene import (Query, Term, TermQuery, TopScoreDocCollector, MultiCollector,
...
class SimpleSearcher(object):
    ...
    def searchWithRequestAndQuery(cls, query, indexReader, taxoReader,
        ...
        returns a List<FacetResult>
        """
        print("\nsearchWithRequestAndQuery: query", type(query))
        ...
        ## for facetRequest in facetRequests:
    ...
    def searchWithDrillDown(cls, indexReader, taxoReader):
        ...
        print("categoryOfInterest: ", categoryOfInterest, type(categoryOfInterest))
...
class FacetExample(object):
    ...
    def __init__(self, directory):
        ...
        self.indexDir = NIOFSDirectory.open(File(os.path.join(self.directory,'Index')))
        self.taxoDir = NIOFSDirectory.open(File(os.path.join(self.directory,'Taxonomy')))

    def createIndex(self):
        # index the sample documents
        print("index the sample documents...")
        SimpleIndexer.index(self.indexDir, self.taxoDir)

    def runSimple(self):
        print("\nsearch the sample documents... (Simple Facet Search)")
        # open readers
    ...
    def runDrillDown(self):
        print("\nsearch the sample documents... (DrillDown Facet Search)")
        # open readers
        taxo = DirectoryTaxonomyReader(self.taxoDir)
        indexReader = IndexReader.open(self.indexDir, True)
